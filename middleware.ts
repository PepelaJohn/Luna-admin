...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    if (!accessResult.allowed) {
      console.log(`[Middleware] Access denied for ${authResult.user?.role} to ${pathname}`);
    ...
    console.log(`[Middleware] Access granted for ${authResult.user?.role} to ${pathname}`);
    return response;
...
async function verifyAuthToken(request: NextRequest, token: string): Promise<AuthResult> {
